# AlphaForge-Lite 项目清单（v0.9 快照 · 妖币百倍天梯版）

> 项目代号：AlphaForge-Lite
> 定位：加密资产"非共识资本聚集探测器"量化验证沙盒
> 快照日期：2026-07-13
> 当前阶段：防洗盘量价背离逃顶通道已打通，15 资产妖币池极限压力测试已交付，单测 34 项全部通过
...
- [x] **本轮新增测试**：`tests/test_divergence.py`（2 项，高压力仿真，
      真实喂入 close/delta2_rs/crowding_penalty 序列走三层过滤网真实计算
      路径，不用显式覆盖值走捷径）：
      - `test_divergence_fake_washout_holds_running_position`："庄家恶性
        假砸盘洗盘"：过滤网一原始信号为真，但换手率全程不拥挤（过滤网二
        不通过），系统必须死守 LEADERSHIP 不被骗出场。
      - `test_divergence_real_breakdown_forces_exit_before_crash`：模拟
        类 LUNA/"币安人生"式真实崩溃前夜，三层过滤全部确认后，打破常规
        迟滞，前瞻性强行切入 EXIT。
      - 同时更新 `tests/test_state_machine_engine.py` 里旧的
        ...
        退出仍由纯拥挤度机制或常规阈值负责。
- [x] 单元测试总计 **34 项全部通过**（较 v0.8 的 32 项：净减 1 项旧测试
      改名复用、新增 2 项高压力仿真测试，符合"新增至少 2 项"要求）
...
修复。单元测试 34 项全部通过。**特别交付了一份诚实声明**，明确指出当前
