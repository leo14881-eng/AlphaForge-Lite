...
- [x] **补充修复：剩余 2 项也已处理完毕，全局扫描发现的问题全部清零**：
      1. `data/download_data.py::_fetch_symbol_ohlcv` 单页请求失败不再
         直接让整个 symbol 的已下载数据一起丢弃——先重试
         `_MAX_PAGE_RETRIES`（3）次线性退避，仍失败则带着已成功拿到的
         部分数据提前返回（不抛异常），调用方打印警告说明数据不完整。
         脚本本身是幂等全量重拉取，重跑一次即可自然补全缺口，"部分
         数据"不会产生脏数据。新增 3 项测试（用 Mock exchange 对象，
         不需要真实网络）：正常翻页、瞬时故障重试后成功、重试耗尽后
         保留部分数据不抛异常。
      2. `run_meme_stress_test.py` 新增 `tests/test_meme_stress_test.py`
         （10 项，此前零覆盖），用真实合成价格序列 + 状态迁移日志实测
         验证了 fork 当时只做代码走读、未实测的两处核心索引/日期算术：
         `_compute_coverage_completeness`（覆盖完整度公式，含"离场恰好
         在峰值"/"涨幅完全回吐"/"未离场用数据末尾兜底"/"从未创新高返回
         None"/"symbol 不存在返回 None"五种场景）、
         `_epic_pool_cross_section`（`peak_idx - entry_idx` 的 Lead Time
         算术、无 EXIT 记录时的窗口终点兜底、零状态迁移记录资产的正确
         跳过、崩溃资产判定用的是全历史回撤而不是入场退场窗口内回撤）。
         **全部 10 项一次性通过，验证了 fork 当初的代码走读判断是对的**
         （没有发现真实 bug，这次是把"读代码觉得对"升级成"跑真实数据
         验证过是对的"）。
- [x] **单元测试总计 96 项全部通过**（83 + 本轮新增 13 项：
      `data/download_data.py` 3 项 + `run_meme_stress_test.py` 10 项）。

**至此，两轮全局扫描（覆盖全部代码文件）发现的问题全部处理完毕**：
10 项发现 全部修复或已判定为非 bug 并补充文档+锁定测试，没有遗留项。
